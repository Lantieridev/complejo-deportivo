...
namespace ComplejoDeportivo.Application.DTOs.Dashboard
{
    public class FiltrosDashboardDto
    {
        // FILTRO PRINCIPAL OBLIGATORIO
        [Required(ErrorMessage = "El complejo es requerido")]
        [Range(1, int.MaxValue, ErrorMessage = "El complejo ID debe ser v�lido")]
        ...
        // FILTROS ESPEC�FICOS DEL COMPLEJO
        [Display(Name = "Cancha")]
        ...
        // CONFIGURACI�N DE PERIODO PREDEFINIDO
        [Display(Name = "Per�odo")]
        public string? PeriodoPredefinido { get; set; }

        // PAGINACI�N (para tablas del dashboard)
        [Range(1, int.MaxValue, ErrorMessage = "La p�gina debe ser mayor a 0")]
        public int Pagina { get; set; } = 1;

        [Range(1, 100, ErrorMessage = "El tama�o de p�gina debe estar entre 1 y 100")]
        ...
        public static ValidationResult? ValidarRangoFechas(DateTime? hasta, ValidationContext context)
        {
            ...
            {
                return new ValidationResult("El rango de fechas no puede ser mayor a 1 a�o");
            }

            return ValidationResult.Success;
        }

        // M�TODOS HELPER ESPEC�FICOS PARA COMPLEJO
        public (DateTime inicio, DateTime fin) ObtenerRangoFechas()
        {
            // Si ya tiene fechas espec�ficas, usarlas
            if (Desde.HasValue && Hasta.HasValue)
                return (Desde.Value.Date, Hasta.Value.Date);

            // Si no, calcular seg�n per�odo predefinido
            ...
            return PeriodoPredefinido?.ToLower() switch
            {
                ...
                _ => (hoy.AddDays(-30), hoy) // Por defecto: �ltimos 30 d�as
            };
        }
        ...
        // M�TODO PARA OBTENER DESCRIPCI�N DE FILTROS
        public string ObtenerDescripcionFiltros(string nombreComplejo)
        {
            ...
            if (TieneFiltrosDeFecha())
            {
                ...
                filtros.Add($"Per�odo: {inicio:dd/MM/yyyy} - {fin:dd/MM/yyyy}");
            }
            ...
            // "Complejo: X" siempre está presente (se agrega arriba incondicionalmente), así que
            // el criterio real de "sin filtros" es que no se haya agregado nada MÁS que eso.
            return filtros.Count > 1 ? string.Join(" | ", filtros) : $"Complejo: {nombreComplejo} (Sin filtros adicionales)";
        }

        // M�TODO PARA LIMPIAR FILTROS (manteniendo el complejo)
        public void LimpiarFiltros()
        {
            ...
            Pagina = 1;
            // No limpiamos ComplejoId ni configuraci�n de paginaci�n/orden
        }

        // M�TODO PARA CREAR FILTROS POR DEFECTO
        ...
        // M�TODO PARA COPIAR FILTROS (�til cuando cambia solo el complejo)
        public FiltrosDashboardDto CopiarConNuevoComplejo(int nuevoComplejoId)
        {
            return new FiltrosDashboardDto
            {
                ...
                Desde = this.Desde,
                Hasta = this.Hasta,
                CanchaId = null, // Resetear cancha espec�fica al cambiar complejo
                TipoCanchaId = this.TipoCanchaId,
                EstadoReservaId = this.EstadoReservaId,
                ClienteId = this.ClienteId,
                AsadorId = null, // Resetear asador espec�fico
